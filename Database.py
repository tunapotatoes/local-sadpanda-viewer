...
class Gallery(base):
    ...
    def __repr__(self):
        return "Gallery name: %s\nPath: %s" % (self.name, self.path)
...
def get_session(requester):
    Database.logger.info("New DB session requested from %s" % requester)
    session = None
    try:
        ...
        session.close()
        Database.logger.info(("DB session ending for %s" % requester))
